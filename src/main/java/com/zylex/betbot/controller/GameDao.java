    ...
    public List<Game> getByRuleNumber(RuleNumber ruleNumber) {
        try (PreparedStatement statement = connection.prepareStatement(SQLGame.GET_BY_RULE_NUMBER.QUERY)) {
            ...
            while (resultSet.next()) {
                ...
                ruleGames.add(new Game(id, league, leagueLink, dateTime, firstTeam, secondTeam, firstWin, tie, secondWin, firstWinOrTie, secondWinOrTie, gameResult, betMade));
            }
            return ruleGames;
...
//    }
    ...
    enum SQLGame {
        ...
        GET_BY_RULE_NUMBER("SELECT id, league, league_link, date_time, first_team, second_team, first_win, tie, second_win, first_win_or_tie, second_win_or_tie, result, bet_made, rule_number FROM game WHERE rule_number = (?)"),
